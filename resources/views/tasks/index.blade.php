{{-- kami menggunakan Komponen AppLayout yang terletak di app\View\Components\AppLayout.php yang menggunakan tampilan resources\views\layouts\app.blade.php --}}
<x-app-layout>
    <!-- Tentukan slot bernama "header" -->
    <x-slot name="header">
        ...
        <div class="flex justify-between">
            ...
            <a href="{{ route('tasks.create') }}" class="btn btn-primary text-white px-4 py-2 rounded-md">ADD</a>
        </div>
    </x-slot>
                        ...
                        <tbody class="bg-white">
                            ...
                            @forelse ($unCompletedTasks as $task)
                                <tr>
                                    ...
                                    <td
                                        class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400 flex items-center">
                                        <!-- Link to edit task -->
                                        <a href="{{ route('tasks.edit', $task->id) }}"
                                            class="btn btn-outline-warning hover:bg-yellow-500 hover:text-white px-4 py-2 rounded-md mr-4">EDIT</a>
                                        <!-- Form to delete task -->
                                        <form method="post" action="{{ route('tasks.destroy', $task->id) }}"
                                            ...
                                            @method('delete')
                                            <button type="submit"
                                                class="btn btn-outline-danger hover:bg-red-500 hover:text-white px-4 py-2 rounded-md h-[35px] relative top-[1px] ml-4">DELETE</button>
                                        </form>
                                    </td>
                                </tr>
                            @empty
                    ...
                    <table class="border-collapse table-auto w-full text-sm">
                        ...
                        <tbody class="bg-white">
                            ...
                            @forelse ($completedTasks as $task)
                                <tr>
                                    <td
                                        class="border-b border-slate-100 dark:border-slate-700 p-4 pl-8 text-slate-500 dark:text-slate-400 justify-center items-center">
